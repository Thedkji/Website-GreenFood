...
class UserSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Tạo dữ liệu giả cho người dùng từ user9 đến user22
        for ($i = 9; $i <= 22; $i++) {
            User::updateOrCreate(
                [
                    'user_name' => 'user' . $i,  // Tạo tên người dùng tự động
                ],
                [
                    'name' => $faker->name(),  // Tạo tên người dùng giả
                    'avatar' => null,  // Avatar có thể null
                    'password' => bcrypt('password'),  // Mật khẩu giả (mã hóa)
                    'email' => $faker->unique()->safeEmail(),  // Email duy nhất
                    'phone' => $faker->regexify('0[0-9]{9}'),  // Số điện thoại giả (đúng 10 chữ số)
                    'province' => $faker->city(),  // Tỉnh giả
                    'district' => $faker->city(),  // Quận giả
                    'ward' => $faker->streetName(),  // Phường giả
                    'address' => $faker->address(),  // Địa chỉ giả
                    'role' => 1,  // Chỉ định là user (mặc định)
                    'email_verified_at' => now(),  // Email đã xác thực
                    'remember_token' => $faker->sha256,  // Mã xác thực người dùng
