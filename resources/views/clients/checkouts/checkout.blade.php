                        ...
                        <div class="col-md-12 col-lg-6">
                            ...
                            <div class="form-item">
                                ...
                                <textarea name="note" class="form-control" spellcheck="false" cols="30" rows="11"
                                    placeholder="Ghi chú (Nếu có)">{{ old('note') }}</textarea>
                                <x-feedback name="note" />
                            </div>
                        </div>
                        <div class="col-md-12 col-lg-6">
                            <div class="form-item">
                                <label for="province" class="form-label my-3">Thành phố <span
                                    ...
                                    @php
                                        if (auth()->check()) {
                                            if ($userInfo->province && $userInfo->province !== null) {
                                                $check = array_filter(
                                                    array_column($provinces, 'ProvinceName'),
                                                    function ($provinceName) use ($userInfo) {
...
// Kiểm tra nếu có tỉnh đã chọn và so sánh
                            ...
                            <div class="form-item">
                                <label for="district" class="form-label my-3">Quận/Huyện <span
                                        class="text-danger">*</span></label>
                                <select name="district" id="district-dropdown" class="form-select"
                                    value="{{ old('district') }}">
                            ...
                            <div class="form-item">
                                <label for="ward" class="form-label my-3">Phường/Xã <span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="form-item">
                                ...
                                <input type="text" class="form-control" id="address" name="address"
                                    value="{{ $userInfo ? $userInfo->address : old('address') }}">
                                <x-feedback name="address" />
                            </div>
                            <div class="form-item">
                                <label class="form-label my-3">Phương thức thanh toán <span
                                        class="text-danger">*</span></label>
                                    ...
                                    <div class="col-12">
                                        <div class="form-check text-start my-3">
                                            <input type="radio" class="form-check-input bg-primary border-0"
                                                id="Paypal-1" name="payment_method" value="VNPay">
                                ...
                                <div class="row g-4 text-center align-items-center justify-content-center border-bottom">
                                    <div class="col-12">
                                        <div class="form-check text-start my-3">
                                            <input type="radio" class="form-check-input bg-primary border-0"
                                                id="Delivery-1" name="payment_method" value="Delivery" checked>
                    ...
                    <div class="col-md-12 col-lg-6 col-xl-6">
                            ...
                            <table class="table text-center align-middle">
                                                ...
                                                @php
                                                    ...
                                                    if (isset($userId)) {
                                                        ...
                                                        if ($item['attributes']['status'] === 0) {
                                                            ...
                                                            $imageSrc .=
                                                                $variantDetails[$item['attributes']['sku']]->img ??
                                                                $item['attributes']['img'];
                                                        }
                                                    }
                                                @endphp
                                                <img src="{{ $imageSrc }}" name="image" class="img-fluid rounded"
                                                ...
                                                <strong>
                                                    @if (isset($userId))
                                                        ...
                                                        @if (!empty($variantDetails[$item['sku']]))
                                                            |
                                                            {{ optional(\App\Models\Variant::find($variantDetails[$item['sku']]->variants[0]['parent_id']))->name }}
                                                            - {{ $variantDetails[$item['sku']]->variants[0]['name'] }}
                                                        @endif
                                                    @else
                                                        {{ $item['name'] }}
                                                        @if (!empty($variantDetails[$item['attributes']['sku']]))
                                                            |
                                                            {{ optional(\App\Models\Variant::find($variantDetails[$item['attributes']['sku']]->variants[0]['parent_id']))->name }}
                                                            -
                                                            {{ $variantDetails[$item['attributes']['sku']]->variants[0]['name'] }}
                                                        @endif
                                                    @endif
                                                </strong><br>
                                                <span class="text-muted"><span class="text-danger"
                                                        id="price_old">{{ number_format($itemPrice) }} VNĐ</span><span
                                                        class="text-danger" id="price_new"></span></span>
                                                <span>X {{ $itemQuantity }}
                                                    <strong>{{ $userInfo ? $item['sku'] : $item['attributes']['sku'] }}</strong></span>
                                            </td>
                                            <td>
                                                <strong
                                                    class="text-primary">{{ number_format($itemPrice * $itemQuantity) }}
                                                    VNĐ</strong>
                                            </td>
                                        </tr>
                                    @endforeach
                                ...
                                <tfoot>
                                    <tr class="table-light">
                                        ...
                                        <td><strong class="text-primary">{{ number_format($totalPrice) }} VNĐ</strong>
                                        </td>
                                    </tr>
                                    ...
                                    <tr class="table-light">
                                        ...
                                        <td><strong class="text-primary" id="totalPrice">{{ number_format($totalPrice) }}
                                                VNĐ</strong></td>
                                    </tr>
                                </tfoot>
                            </table>
                        ...
                        <input type="hidden" name="total"
                            value="{{ session('coupon') ? $totalPriceCoupon : $totalPrice }}">
            ...
            <div id="form-coupon" class="row text-center align-items-center justify-content-center">
                ...
                <div class="d-flex flex-row mb-3 gap-3 mt-3">
                    <input type="text" name="coupon_name" class="form-control" placeholder="Nhập mã giảm giá"
                        value="{{ request()->input('coupon_name') }}">
                    <select class="form-select" aria-label="Coupon selection" name="coupon_id" id="coupon_id">
                        <option value="" disabled {{ !session('coupon') ? 'selected' : '' }}>Chọn mã giảm giá
                        </option>
                ...
                <button class="btn border-secondary text-primary py-3 px-4" id="applyCouponBtn">Áp dụng</button>
            </div>
        </div>
    </div>
    </div>
