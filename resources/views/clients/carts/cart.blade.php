...
@section('content')
    <style>
        ...
        .form-check-input[type="checkbox"].checkbox {
            border: 3px solid #81c408 !important;
        }
    ...
    <div class="container-fluid py-5">
        ...
        <div class="container py-5">
            <form action="{{ route('client.checkout') }}" method="get">
                @csrf
                @if (session('check'))
                    <button type="submit" class="btn btn-primary" formaction="{{ route('client.removeCheck') }}"
                        formmethod="post">Cập nhật lại</button>
                @endif
                    ...
                    <table class="table">
                        <thead class="table-header">
                            <tr class="text-center">
                                <th scope="col">
                                    <input type="checkbox" class="form-check-input checkbox border-0"
                                        style="width: 20px;height: 20px;" id="select-all" onclick="toggleSelectAll(this)">
                                </th>
                                <th scope="col">Ảnh</th>
                        ...
                        <tbody class="table-body text-center">
                            @if ($cartItems->isNotEmpty())
                                    ...
                                    @if (auth()->check())
                                        @php
                                            ...
                                            if ($item->product->status === 0) {
                                                ...
                                                isset($variantGroups[$item->sku]) &&
                                                $variantGroups[$item->sku]->isNotEmpty()
                                            ) {
                                        ...
                                        @endphp
                                        <tr>
                                            <th class="align-content-center cursor-pointer">
                                                <input type="checkbox" name="selectBox[{{ $item->id }}]"
                                                    onclick="toggleDeleteButton()"
                                                    class="form-check-input checkbox border-0 cart-checkbox"
                                                    data-item-id="{{ $item->id }}" data-price="{{ $price }}"
                                                    style=" width: 20px;height: 20px;" value="{{ $item }}"
                                ...
                                disabled @endif
                                    ...
                                    <th>
                                        <div class="d-flex align-items-center justify-content-center">
                                            ...
                                            <a href="{{ route('client.product-detail', $item->product->id) }}">
                                                <img src="{{ $imageSrc }}" class="img-fluid mx-auto rounded-circle"
                                    ...
                                    <td>
                                        <a href="{{ route('client.product-detail', $item->product->id) }}">
                                            <p class="mb-0 truncate-text align-content-center truncate m-auto"
                                                data-fulltext="{{ $item->product->name }}">
                                                {{ $item->product->name }}
                                    ...
                                    <td>
                                        <div class="input-group quantity mt-4 mx-auto" style="width: 100px;">
                                            ...
                                            <input type="text" id="quantity_{{ $item->sku }}"
                                ...
                                @else
                                    ...
                                    <tr>
                                        <th>
                                            <input type="checkbox"
                                                class="form-check-input bg-primary border-0 cart-checkbox"
                                                style="width: 20px;height: 20px;" name="selectBox[{{ $item->id }}]"
                                                value="{{ $item }}" data-price="{{ $price }}"
                                                @if (!empty($lowStockVariants)) @foreach ($lowStockVariants as $stock)
                                ...
                                <th scope="row">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <a href="{{ route('client.product-detail', $item->attributes->product_id) }}">
                                            <img src="{{ $imageSrc }}" class="img-fluid me-5 rounded-circle"
                                                style="width: 80px; height: 80px;" alt="">
                                        </a>
                                    </div>
                                </th>
                                ...
                                <td>
                                    <div class="input-group quantity mt-4" style="width: 100px;">
                                        ...
                                        <input type="text" id="quantity_{{ $item->attributes->sku }}"
                                            name="quantities[{{ $item->id }}]"
                                            class="form-control form-control-sm text-center border-0"
                                            value="{{ $item->quantity }}">
                                        <input type="hidden" name="priceTotal[{{ $item->id }}]">
