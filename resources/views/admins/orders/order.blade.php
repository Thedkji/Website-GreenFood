...
<form action="{{ route('admin.orders.showOder') }}" id="search-form" method="GET"
    ...
    @if (request('statusFilter') === '5')
    <div class="col-sm">
        <label for="">Phương thức thanh toán</label>
        <select name="payment_method" class="form-select" onchange="this.form.submit()">
            <option value="">Tất cả phương thức</option>
            <option value="0" {{ request('payment_method') === '0' ? 'selected' : '' }}>Tiền mặt</option>
            <option value="1" {{ request('payment_method') === '1' ? 'selected' : '' }}>Thanh toán VN Pay
            </option>
        </select>
    </div>
    @endif
    <div class="col-sm">
        ...
        <select id="statusFilter" name="statusFilter" class="form-select" onchange="this.form.submit()">
            ...
            <option value="3" {{ request('statusFilter') === '3' ? 'selected' : '' }}>Giao hàng thành công
            </option>
            ...
            <option value="6" {{ request('statusFilter') === '6' ? 'selected' : '' }}>Đánh giá</option>
            <!-- <option value="7" {{ request('statusFilter') === '7' ? 'selected' : '' }}>Hoàn thành</option> -->
        </select>
    </div>
</form>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td scope="col" class="truncate-text">
                        <span class="truncate"
                            data-fulltext="{{ $order->order_code }}">{{ $order->order_code }}</span>
                    </td>
                    <td scope="col" class="truncate-text">
                        <span class="truncate"
                    ...
                    <td scope="col" class="truncate-text">{{ $order->email }}</td>
                    ...
                    <td>
                        ...
                        @else
                        <span class="badge bg-primary p-2">VN Pay</span>
                        @endif
                    </td>
                    <td scope="col" class="truncate-text truncate" data-fulltext="{{ $order->created_at->format('d-m-Y H:i:s') }}">{{ $order->created_at->format('d-m-Y H:i:s') }}</td>
                    <td>
                        <div class=" gap-3 flex-wrap">
                            <a href="{{ route('admin.orders.showOrderDetail', $order->id) }}"
                                class="truncate" data-fulltext="Xem chi tiết"><i
